...
export function ExplanationPanel({ explanation }: { explanation: ExplanationPayload | null | undefined }) {
  if (!explanation) {
    return <div style={{ color: "#666" }}>No explanation available</div>;
  }
  ...
  return (
    <div style={{ display: "grid", gap: 10 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <strong>{explanation.target_power_kw.toFixed(1)} kW</strong>
      </div>
      <DataTable
      ...
      />
      <blockquote style={{ margin: 0, padding: "8px 10px", borderLeft: "4px solid #90caf9", background: "#f7fbff" }}>{explanation.summary}</blockquote>
    </div>
  );
}
